...
module Categories.Adjoint.Slice {o l e o′ l′ e′} {K : Category o l e} {H : Category o′ l′ e′}
    ...
    G~ = record
      ...
      let open Category H
          ...
          begin
              (G⊣U.counit.η A ∘ G.₁ (SliceObj.arr (Slice⇒.cod arr))) ∘ G.₁ h ≈⟨ pullʳ (Equiv.sym G.homomorphism) ⟩
              G⊣U.counit.η A ∘ G.₁ (SliceObj.arr (Slice⇒.cod arr) K.∘ h)      ≈⟨ refl⟩∘⟨ G.F-resp-≈ f ⟩
              G⊣U.counit.η A ∘ G.₁ (SliceObj.arr (Slice⇒.dom arr))            ∎ }
      ; identity = G.identity
    ...
    U~ = record
      ...
      let open Category K
        ...
        begin
          U.₁ (SliceObj.arr (Slice⇒.cod arr)) ∘ U.₁ h ≈⟨ Equiv.sym U.homomorphism ⟩
          U.₁ (SliceObj.arr (Slice⇒.cod arr) H.∘ h) ≈⟨ U.F-resp-≈ f ⟩
          U.₁ (SliceObj.arr (Slice⇒.dom arr)) ∎ }
      ; identity = U.identity
