...
module FMoore.Limits2 {o l e} {C : Category o l e} (F : Functor C C) (O : Category.Obj C)
  ...
  Rδ : (A : FMealyObj) (i : ℕ) → E A ⇒ R ^ i $ O
  Rδ A zero = Ladjunct (s A)
  Rδ A (suc i) = Ladjunct (Rδ A i ∘ d A)

  module R∞ = IndexedProductOf R∞

  behaviour : (A : FMealyObj) → E A ⇒ R∞.X
  behaviour A = R∞.⟨ Rδ A ⟩
  ...
  Terminal-FMealy : Terminal FMealy
  Terminal-FMealy = record
    { ⊤ = record
      { E = R∞.X
      ; d = d∞
      ; s = Radjunct (R∞.π 0)
    ...
    ; ⊤-is-terminal = record
      { ! = λ {A} → let module A = FMealyObj A in record
        { hom = behaviour A
        ; comm-d =
            begin
              R∞.⟨ Rδ A ⟩ ∘ A.d
              ...
              R∞.⟨ (λ i → (Radjunct (R∞.π (ℕ.suc i))) ∘ F.₁ (behaviour A)) ⟩
                ≈⟨ ⟺ (R∞.⟨⟩∘ _ (F.₁ (behaviour A))) ⟩
              d∞ ∘ F.₁ (behaviour A) ∎
        ; comm-s = p2rof (Equiv.sym (R∞.commute (Rδ A) 0))
        }
      ; !-unique = λ {A} f → R∞.unique _ _ (uniqueness f)
      }
    } where

      module _ {A : FMealyObj} where
        private module A = FMealyObj A

        module _ (f : FMealy⇒ A _) where
          private module f = FMealy⇒ f
          uniqueness : (i : ℕ) → R∞.π i ∘ f.hom ≈ Rδ A i
          uniqueness ℕ.zero = prof (Equiv.sym f.comm-s)
  ...
  module ⊤ = Terminal Terminal-FMealy
  module _ {A B : FMealyObj} where
    private
    ...
    module _ {G : FMealyObj} (PA : FMealy⇒ G A) (PB : FMealy⇒ G B) where
      private
        module G  = FMealyObj G
        module PA = FMealy⇒ PA
        module PB = FMealy⇒ PB
      ...
      abstract
        ...
        universal-⟨-,-⟩ = begin
          ...
          where
            ...
            universal-⟨-,-⟩-pointwise ℕ.zero = {!  !} -- ⟺ (p2rof ((Equiv.sym PA.comm-s ○ PB.comm-s)))
  ...
  module _ {A G : FMealyObj} (PA :  FMealy⇒ G A) where
    private
      module G  = FMealyObj G
      module A  = FMealyObj A
      module PA = FMealy⇒ PA
    ...
    abstract
      ...
      commute-behaviour =
        ...
        where
          ...
          commute-behaviour-pointwise ℕ.zero = {!  PA.comm-s !} --PA.comm-s
  ...
  BinaryProducts-FMealy : BinaryProducts FMealy
  BinaryProducts-FMealy = record
    { product = λ { {A} {B} →
    let module A = FMealyObj A
        module B = FMealyObj B in
      record
        { A×B = record
          { E = P∞.P
          ; d = P∞.universal {_} {_} {F.F₀ P∞.P} {A.d ∘ F.₁ P∞.p₁} {B.d ∘ F.₁ P∞.p₂} universal-d
          ; s = {!  R∞.π 0 ∘ P∞.diag !} --R∞.π 0 ∘ P∞.diag
          }
        ; π₁ = record
          { hom = P∞.p₁
        ...
        ; ⟨_,_⟩ =
            ...
            --begin
            --    s C                                 ≈⟨ comm-s (⊤.! {C}) ⟩
            --    R∞.π 0 ∘ behaviour C                ≈⟨ refl⟩∘⟨ commute-behaviour PA ⟩
